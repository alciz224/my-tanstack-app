...
export const createUser = createServerFn({ method: 'POST' })
  .validator((input: { email: string; name: string; role?: 'admin' | 'user' }) => input)
  .handler(async ({ data }) => {
    const dataService = getDataService();
...
export const updateUser = createServerFn({ method: 'POST' })
  .validator((input: { id: string; data: Partial<{ email: string; name: string; role: 'admin' | 'user' }> }) => input)
...
export const createProduct = createServerFn({ method: 'POST' })
  .validator((input: { name: string; description: string; price: number; category: string }) => input)
  .handler(async ({ data }) => {
    const dataService = getDataService();
...
export const updateProduct = createServerFn({ method: 'POST' })
  .validator((input: { id: string; data: Partial<{ name: string; description: string; price: number; inStock: boolean }> }) => input)
  .handler(async ({ data: { id, data } }) => {
    const dataService = getDataService();
