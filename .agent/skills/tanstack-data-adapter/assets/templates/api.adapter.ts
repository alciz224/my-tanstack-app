...
export class ApiAdapter extends BaseAdapter implements DataAdapter {
  ...
  async createUser(input: CreateUserInput): Promise<User> {
    return this.fetch<User>('/users', { method: 'POST', body: JSON.stringify(input) });
  }

  async updateUser(id: string, data: Partial<CreateUserInput>): Promise<User> {
    return this.fetch<User>(`/users/${id}`, { method: 'PATCH', body: JSON.stringify(data) });
  }
  ...
  async createProduct(input: Omit<Product, 'id'>): Promise<Product> {
    return this.fetch<Product>('/products', { method: 'POST', body: JSON.stringify(input) });
  }

  async updateProduct(id: string, input: UpdateProductInput): Promise<Product> {
    return this.fetch<Product>(`/products/${id}`, { method: 'PATCH', body: JSON.stringify(input) });
  }
