...
import { History, AlertCircle, X, CheckCircle2 } from 'lucide-react'
import { useRouter } from '@tanstack/react-router'

interface GenerateTranscriptsModalProps {
  isOpen: boolean
  onClose: () => void
}

export function GenerateTranscriptsModal({
  isOpen,
  onClose,
}: GenerateTranscriptsModalProps) {
  ...
  // Mocks for options
  const schoolYears = [
    { id: 'sy-2', name: '2024-2025' },
    { id: 'sy-1', name: '2023-2024' },
  ]
  const classrooms = [
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!formData.schoolYearId || !formData.classroomId) {
      setError('Veuillez sélectionner une année et une classe.')
      return
    }
