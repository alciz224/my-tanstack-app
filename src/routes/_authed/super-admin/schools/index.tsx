...
function SchoolsListPage() {
  ...
  const schools = [
    { id: 'SCH-001', name: 'Lycée Albert Camus', type: 'Public', ire: 'Conakry', dpe: 'Ratoma', status: 'Active', studentsCount: 2450 },
    { id: 'SCH-002', name: 'Groupe Scolaire Ousmane Camara', type: 'Privé', ire: 'Conakry', dpe: 'Dixinn', status: 'Active', studentsCount: 1200 },
    { id: 'SCH-003', name: 'Collège Château d\'Eau', type: 'Public', ire: 'Kindia', dpe: 'Kindia', status: 'Inactive', studentsCount: 850 },
    { id: 'SCH-004', name: 'Complexe Scolaire Saint-Georges', type: 'Privé', ire: 'Conakry', dpe: 'Matoto', status: 'Active', studentsCount: 3100 },
    { id: 'SCH-005', name: 'Lycée Waly Diarrah', type: 'Public', ire: 'Labé', dpe: 'Labé', status: 'Active', studentsCount: 1900 },
  ]
    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Supervisez les établissements scolaires à travers la Guinée.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {schools.map((school) => (
          ...
          >
            ...
            <h3 className="text-xl font-bold text-foreground mb-1 group-hover:text-primary transition-colors">{school.name}</h3>
            <p className="text-sm font-mono text-muted-foreground mb-4">{school.id}</p>
            
            <div className="space-y-3 mt-auto pt-4 border-t border-border">
              <div className="flex items-center text-sm text-foreground/80">
                <MapPin className="w-4 h-4 mr-2 text-muted-foreground" />
                <span>IRE {school.ire} • DPE {school.dpe}</span>
              </div>
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium text-foreground">{school.studentsCount.toLocaleString('fr-FR')}</span>
              </div>
            </div>
          </Link>
        ))}
