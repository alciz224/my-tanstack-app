...
import { GraduationCap, Plus, Search, Filter, Edit, Loader2 } from 'lucide-react'
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Gestion des Niveaux</h1>
          <p className="text-muted-foreground mt-1">Définissez les niveaux scolaires associés à chaque cycle.</p>
        </div>
      ...
      <div className="bg-card border border-border rounded-lg p-4 flex flex-col sm:flex-row gap-4 hover-lift">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Rechercher un niveau..." className="w-full bg-input border-none rounded-lg py-2.5 pl-10 pr-4 text-foreground focus:ring-2 focus:ring-ring focus:outline-none transition-shadow" />
        </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              <th className="px-6 py-4 text-[11px] font-semibold uppercase tracking-wide text-foreground/80">Nom du Niveau</th>
              <th className="px-6 py-4 text-[11px] font-semibold uppercase tracking-wide text-foreground/80">Cycle Associé</th>
          ...
          <tbody className="divide-y divide-border">
            ...
            {levels?.map((level) => (
              <tr key={level.id} className="hover:bg-muted/40 transition-colors group">
                ...
                <td className="px-6 py-4 text-foreground/80">{level.cycle_id}</td>
                <td className="px-6 py-4">
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-semibold uppercase tracking-wide bg-success/15 text-success`}>
                    Active
                  </span>
                </td>
