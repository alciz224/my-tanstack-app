...
import { Shield, Plus, MoreVertical, Edit2, Loader2 } from 'lucide-react'
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Gestion des Cycles</h1>
          <p className="text-muted-foreground mt-1">Configurez les cycles d'enseignement du système éducatif guinéen.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        {cycles?.map((cycle) => (
          <div key={cycle.id} className="bg-card border border-border rounded-lg p-6 hover-lift flex flex-col group">
            ...
            <h3 className="text-xl font-bold text-foreground mb-1 group-hover:text-primary transition-colors">{cycle.name}</h3>
            <p className="text-sm text-muted-foreground mb-4">Durée: <span className="font-medium text-foreground">{cycle.duration}</span></p>
