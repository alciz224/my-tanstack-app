...
function TimetableBuilderPage() {
  ...
  const [scheduleBlocks, setScheduleBlocks] = useState<Schedule[]>(
    loaderData.schedule,
  )
  ...
  const handleCreate = async () => {
    ...
    const assignment = MOCK_TEACHER_ASSIGNMENTS.find(
        ...
        {MOCK_TEACHER_ASSIGNMENTS.map((a) => (
          ...
          >
            <span
              ...
              style={{ backgroundColor: a.subject_color }}
            />
            {a.subject_name}
          </div>
        ))}
          ...
          {timeSlots.map((slot) => {
            ...
            return (
              ...
              >
                {/* Time label */}
                <div
                  className={`w-20 flex-shrink-0 border-r border-border p-2 flex flex-col items-center justify-center ${isBreak ? 'bg-amber-500/5' : 'bg-muted/5'} relative`}
                >
                  {isBreak ? (
                    ...
                    <>
                      ...
                      <span className="text-[10px] text-muted-foreground my-0.5">
                        {slot.name}
                      </span>
                      <span className="font-bold text-xs text-foreground">
                        {slot.end_time}
                ...
                {DAYS.map((day) => {
                  ...
                  if (isBreak) {
                    return (
                      <div
                        key={`${day.id}-${slot.id}`}
                        className="flex-1 border-r border-border last:border-r-0 bg-amber-500/5 flex items-center justify-center"
                      >
                        <span className="text-xs text-amber-600/50 dark:text-amber-400/40 font-medium">
                          —
                        </span>
                      </div>
                    )
                    ...
                    >
                      {block ? (
                        <button
                          ...
                          style={{
                            backgroundColor: `${block.subject_color}18`,
                        ...
                        <button
                          onClick={() =>
                            handleCellClick(day.id, slot.id, slot.name)
                          }
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-lg text-sm">
                ...
                <span className="text-foreground font-medium">
                  {DAYS.find((d) => d.id === modal.day)?.label} —{' '}
                  {modal.slotName}
                </span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-background rounded-xl shadow-2xl max-w-sm w-full overflow-hidden">
            <div
              ...
              style={{
                borderLeftWidth: 4,
                borderLeftColor: modal.block.subject_color,
              }}
            >
              <div className="flex items-center gap-3">
                <span
                  className="w-3 h-3 rounded-full"
                  style={{ backgroundColor: modal.block.subject_color }}
                />
                <h2 className="font-bold text-lg text-foreground">
                  {modal.block.subject_name}
                </h2>
              </div>
              <button
                onClick={() => {
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-foreground font-medium">
                    {modal.block.teacher_name}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <Clock className="w-4 h-4 text-muted-foreground" />
                  <span className="text-foreground">
                    {modal.block.start_time} – {modal.block.end_time}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <CalendarIcon className="w-4 h-4 text-muted-foreground" />
                  <span className="text-foreground">
                    {DAYS.find((d) => d.id === modal.block.day_of_week)?.label}
                  </span>
                </div>
              </div>
              <div className="flex items-center justify-between pt-2 border-t border-border">
                <span
                  className={`px-2 py-0.5 rounded-full text-xs font-semibold ${modal.block.status === 'ACTIVE' ? 'bg-green-500/15 text-green-700 dark:text-green-400' : 'bg-muted text-muted-foreground'}`}
                >
                  {modal.block.status}
                </span>
                <button
