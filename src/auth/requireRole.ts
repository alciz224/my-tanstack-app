...
export function requireRole(user: User, allowedRoles: Array<UserRole>) {
    if (import.meta.env.DEV) {
        console.debug('[requireRole] user.role:', user.role, '| allowedRoles:', allowedRoles, '| available_roles:', user.available_roles)
    }

    if (!user.role) {
        if (import.meta.env.DEV) console.debug('[requireRole] → redirect /select-portal (role is null)')
    ...
    if (!allowedRoles.includes(user.role)) {
        if (import.meta.env.DEV) console.debug('[requireRole] → redirect /unauthorized (role not in allowedRoles)')
        throw redirect({ to: '/unauthorized' })
    }

    if (!user.available_roles.includes(user.role)) {
        if (import.meta.env.DEV) console.debug('[requireRole] → redirect /unauthorized (role not in available_roles)')
        throw redirect({ to: '/unauthorized' })
    }
